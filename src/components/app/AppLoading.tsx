...
export function AppLoading({ locale = "ko" }: { locale?: AppLocale }) {
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#f4f6f5] px-4 text-[#18211d]">
      <section className="w-full max-w-xl rounded-lg border border-[#d8dfda] bg-white p-5 text-center shadow-sm">
        ...
        <p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-[#64756d]">{copy.description}</p>
        <p className="mt-4 text-sm font-medium text-[#4f6258]">{copy.status}</p>
      </section>
    </main>
  );
}
